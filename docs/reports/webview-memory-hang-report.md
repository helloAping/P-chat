...
- **B. 超时更聪明**（部分已实施，2026-08-03）：**自动重试**已落地——新增 `limits.max_turn_retries`（默认 2，0 = 关闭），回合因 `max_turn_seconds` 被终止时，服务端在同一 SSE 流内重新加载已持久化的部分进度 + 自动注入一条"继续"消息（等价于用户手动发送"继续"），重新获得完整预算再跑一轮，全部重试耗尽后才输出"回合超出最长执行时间"终止帧。尚未实施：超时前发"即将超时"SSE 事件让 LLM 收尾；超时时先优雅写回 todo 状态再终止。
