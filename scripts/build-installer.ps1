...
foreach ($f in @($guiExe, $serverExe, $cliExe)) {
    if (-not (Test-Path -LiteralPath $f)) {
        Write-Error "缺少二进制文件: $f — 请先运行 task build && task build:gui"
        exit 1
    }
}

# --- copy assets ---
Write-Host "[build-installer] 复制二进制文件 → $assets"
...
Write-Host "[build-installer] 复制 web/ → $assets\web"
...
Write-Host "[build-installer] 复制安装脚本"
